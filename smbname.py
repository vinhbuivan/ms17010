...
NetBIOS_ITEM_TYPE = {
    b'\x01\x00':'Computer name',
    b'\x02\x00':'Domain name',
    b'\x03\x00':'DNS name',
...
if len(sys.argv) == 2:
    ...
    threads = [ threading.Thread(target=smb_detect, args=(addr, 445)) for addr in addrs]
    for t in threads:
        t.start()
        time.sleep(0.1)
    for t in threads:
    ...
    if sys.argv[2] == '139' or sys.argv[2] == '445':
        ...
        print('Usage: python netbios.py ip [port 139|445]')
else:
    print('Usage: python netbios.py ip [port 139|445]')
while isAlive > 0:
    time.sleep(1)
