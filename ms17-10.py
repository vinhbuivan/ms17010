...
#                    logger.info("[+] "+str(targets)+"\t"+nativeos)
                ...
                if data[8:10] == "\x25\x05":
                    ...
                    if data[9:13] == "\x05\x02\x00\xc0":
                        logger.info("[+] "+str(targets)+"\t"+nativeos+"\t VULNERABLE to MS17-010")
                        vuln = 1
        ...
        if SingleMultiScanCheck == 2:
            logger.error("[+] Can't connecto to "+str(targets))
            sys.exit(1)
...
if SingleMultiScanCheck == 1:
    ...
    for i in net4.hosts():
        if str(i)[-2:] != ".0" and str(i)[-4:] != ".255":
            totip += 1
            t = threading.Thread(target=conn, args=(i, logger,))
            threads.append(t)
            t.start()
            time.sleep(0.01)
